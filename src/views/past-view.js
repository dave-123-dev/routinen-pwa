...
export class PastView {
  ...
  ensureUi() {
    if (!document.getElementById('past-view-style')) {
      ...
      style.textContent = `
        ...
        .viewTabs button,.compactToggle,.loadMorePast{border:1px solid var(--line);background:var(--surface);color:var(--text);border-radius:999px;padding:12px 16px;font-weight:800}
        ...
        body.compact #list .card .meta,body.compact #list .card .status,body.compact #list .card .complete,body.compact #list .card .delSwipe{display:none!important}
        body.compact #list .card .taskTop{display:contents}
        body.compact #list .card .iconbox{width:72px;height:72px;font-size:32px}
        body.compact #list .card .titleRow{display:flex}
  ...
  render() {
    ...
    $('compactToggle').textContent = this.compact ? text.normal : text.compact;
